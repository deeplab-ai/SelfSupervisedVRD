...
class RankingClsEvaluator:
    ...
    def print_stats(self):
        ...
        for rmode in ('micro', 'macro'):
            acc = self._compute_acc(rmode)
            print('Ranking %sAccuracy:' % (rmode), acc)
            f.write(f'Ranking {rmode}Accuracy:  {acc} \n')
        f1_score_tmp = 100 * np.mean(self._f1_scores)
        print('Ranking f1-score:', f1_score_tmp)
        f.write(f'Ranking f1-score: {f1_score_tmp} \n')
        f.write(' \n')
        f.close()
