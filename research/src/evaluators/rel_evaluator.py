...
class RelationshipEvaluator:
    ...
    def __init__(self, annotation_loader, use_merged=False, net_name='', graph_constraints=70):
        ...
        print(f"Graph Constraints: {self.graph_constraints}")
    ...
    def print_stats(self, task, net_info):
        ...
        for rmode in ('micro', 'macro'):
            for cmode in ('graph constraints', 'no constraints'):
                for dmode in ('full', 'zeroshot'):
                    f.write(
                        f'{rmode}Recall@20-50-100 {task} {dmode} with {cmode}: '
                        f'{self._compute_recall(rmode, cmode, dmode)} \n'
                    )
                    print(
    ...
    def _sort_detected(self, scores, boxes, labels,
        ...
        if graph_constraints:  # only one prediction per pair
            classes = classes[:, :1]
            scores = scores[:, :1]
        else:  # k = 5 instead of k = 70
            classes = classes[:, :self.graph_constraints]
            scores = scores[:, :self.graph_constraints]
