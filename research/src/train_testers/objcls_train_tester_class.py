...
class ObjClsTrainTester(BaseTrainTester):
    ...
    def test(self):
        ...
        if self.test_on_masked:
            self.logger.debug("Test only on masked tokens")
        self.net.eval()
        ...
        for batch in tqdm(self.data_loader):
            for step in range(len(batch['filenames'])):
                masked_obj_ids = None
                scores = self._net_outputs(batch, step)
                if type(scores) is tuple:
                    scores, masked_obj_ids = scores[0], scores[1]
                    masked_obj_ids = masked_obj_ids.cpu().numpy() if masked_obj_ids is not None and self.test_on_masked else None
                    evaluator.mask_obj_ids = masked_obj_ids
                scores = scores.cpu().numpy()
                filename = batch['filenames'][step]
                evaluator.step(filename, scores)
                if masked_obj_ids is not None and self.test_on_masked:
                    scores = scores[masked_obj_ids]
        ...
        evaluator.print_stats(self._net_name, self.net_info)
        with open(self._results_path + 'results.json', 'w') as fid:
            json.dump(results, fid)

    def _compute_loss(self, batch, step):
        """Compute loss for current batch."""
        # scores, mask_idx_obj = self._net_forward(batch, step)
