...
class BaseTrainTester:
    ...
    def _set_from_config(self, config):
        ...
        self.normal_training = config.normal_training
    ...
    def train_test(self):
        ...
        # don't include the weights of the fine-tune layers
        for name, param in self.net.named_parameters():
            if 'fc_objects' in name or 'classifier' in name:
    ...
    def train(self):
        ...
        self.logger.info(self._net_name)
    ...
    def _train_epoch(self, epoch, model_path_name):
        ...
        if self.lb_imgs:
            for lb_batch, unlb_batch in tqdm(zip(self.lb_data_loader, self.unlb_data_loader), total=len(self.unlb_data_loader)):

                for key in lb_batch.keys():
                    for sample in unlb_batch[key]:
                        lb_batch[key].append(sample)
                batch = copy.deepcopy(lb_batch)
