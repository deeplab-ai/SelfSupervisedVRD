...
class SGGTrainTester(BaseTrainTester):
    ...
    def __init__(self, net, config, features, obj_classifier=None,
        ...
        self.graph_constraints = config.graph_constraints
        self.lb_imgs = config.lb_imgs
        self.net_info = {
            'random_seed': config.random_seed if config.random_seed else 'None',
            'few_shot': config.k_shot if config.k_shot else 'None',
            'pretrained_net': config.pretrained_model if config.pretrained_model else 'None'
        }
    ...
    def test(self):
        ...
        if self._test_on_negatives:
            ...
            rel_eval = RelationshipEvaluator(self.annotation_loader,
                                             self._use_merged, self._net_name, self.graph_constraints)
        ...
        rel_eval.print_stats(self._task, self.net_info)
        if rank_scores is not None and self._task == 'predcls':
    ...
    def _compute_loss(self, batch, step):
        ...
        if self.teacher is not None and self._negative_loss is not None:
            if self._neg_classes is not None:
                ...
                neg_classes = [i for i in range(self.num_rel_classes - 1)]
    ...
    def _consistency_loss(self, batch, step, scores, typ='triplet_sm'):
        ...
        if 'triplet_sm' in typ:
            ...
            return loss_cons.mean()  # , loss_bias.mean()

        elif 'kl_loss' in typ:
            subj_maps = t_outputs[2]
            obj_maps = t_outputs[3]
            soft_scores = F.softmax(scores, dim=1)
            subj_maps = torch.clamp(subj_maps.flatten(2), min=1e-8)
            obj_maps = torch.clamp(obj_maps.flatten(2), min=1e-8)
            subj_masks = subj_masks.unsqueeze(1).expand(
                -1, self.net.num_rel_classes, -1)
            obj_masks = obj_masks.unsqueeze(1).expand(
                -1, self.net.num_rel_classes, -1)
            subj_kl_dist = F.kl_div(torch.log(subj_maps),
                                    subj_masks, reduction='none').mean(-1)
            obj_kl_dist = F.kl_div(torch.log(obj_maps),
                                   obj_masks, reduction='none').mean(-1)
            subj_sims = F.softmin(5 * subj_kl_dist, dim=1)
            obj_sims = F.softmin(5 * obj_kl_dist, dim=1)
            # Mode-seeking KL-div
            subj_loss_md = F.kl_div(torch.log(subj_sims + 1e-8), soft_scores,
                                    reduction='none').mean(-1)
            obj_loss_md = F.kl_div(torch.log(obj_sims + 1e-8), soft_scores,
                                   reduction='none').mean(-1)
            # # Mean-seeking KL-div
            # subj_loss_mn = F.kl_div(torch.log(subj_sims + 1e-8), soft_scores,
            #                      reduction='none').mean(-1)
            # obj_loss_mn = F.kl_div(torch.log(obj_sims + 1e-8), soft_scores,
            #                     reduction='none').mean(-1)
            subj_loss = subj_loss_md
            obj_loss = obj_loss_md
            ...
            soft_scores = F.softmax(scores, dim=1)
            subj_weighted_maps = (
                    subj_maps * soft_scores.unsqueeze(-1).unsqueeze(-1)
            ).sum(1).flatten(1)
            obj_weighted_maps = (
                    obj_maps * soft_scores.unsqueeze(-1).unsqueeze(-1)
            ).sum(1).flatten(1)
            subj_loss = F.binary_cross_entropy(
                subj_weighted_maps, subj_masks, reduction='none').mean(-1)
            obj_loss = F.binary_cross_entropy(
                obj_weighted_maps, obj_masks, reduction='none').mean(-1)
        return subj_loss[unlabeled].mean()  # + obj_loss[unlabeled]
    ...
    def _net_forward(self, batch, step):
        ...
        if self.lb_imgs:
            return self.net(
                batch['images'][step].to(self._device),
                batch['object_rois'][step].to(self._device),
                batch['object_ids'][step].to(self._device),
                batch['pairs'][step],
                batch['image_info'][step]
            )
        else:
            return self.net(
                self.data_loader.get('images', batch, step),
    ...
    def _set_data_loaders(self, mode_ids={'train': 0, 'val': 1, 'test': 2}):
        ...
        if self.lb_imgs and 'train' in mode_ids:
            labeled = np.array([anno['labeled'] for anno in annotations])
        datasets = {
            split: SGGDataset(
        ...
        if self.lb_imgs and 'train' in mode_ids:
            self.logger.debug("Spiting dataset to labeled and unlabeled images..")
            dataset_lb = SGGDataset(
                annotations[labeled == 1].tolist(),
                self.config, self.features)
            dataset_unlb = SGGDataset(
                annotations[labeled == 0].tolist(),
                self.config, self.features)
        ...
        if self.lb_imgs and 'train' in mode_ids:
            factor = (len(datasets['train']) / self.lb_imgs) - 1
            self.logger.debug(f"Running unlabeled dataset with {self._batch_size} batch size.")
            self._data_loaders['unlabeled'] = SGGDataLoader(
                dataset_unlb, batch_size=self._batch_size,
                shuffle=True, num_workers=self._num_workers,
                drop_last='train' != 'test', device=self._device)
            self.logger.debug(f"Running labeled dataset with {int(self._batch_size // factor)} batch size.")
            self._data_loaders['labeled'] = SGGDataLoader(
                dataset_lb, batch_size=int(self._batch_size // factor),
                shuffle=True, num_workers=self._num_workers,
                drop_last='train' != 'test', device=self._device)
