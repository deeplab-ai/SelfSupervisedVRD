...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.lb_imgs:
            return_json['labeled'] = anno['labeled']
        return return_json
    ...
    def _set_init(self):
        ...
        self.lb_imgs = self._config.lb_imgs
...
class SGGDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=True, num_workers=2,
                 drop_last=False, device='cuda:0'):  # worker_init_fn=np.random.seed(1)):
        """Initialize loader for given dataset and annotations."""
        super().__init__(
            dataset, batch_size=batch_size, shuffle=shuffle,
            num_workers=num_workers, drop_last=drop_last,
            collate_fn=lambda data: sgg_collate_fn(data))# , worker_init_fn=worker_init_fn)
        self._device = device
        # torch.manual_seed(1)
