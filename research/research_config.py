...
class ResearchConfig(Config):
    ...
    def __init__(self, net_name='', phrase_recall=False, test_dataset=None,
                 ...
                 misc_params=None, finetune=False, normal=True, pretrained_model=None, k_shot=None,
                 random_seed=None, mask_ratio=0.5, pretrain_task=None, lb_imgs=None,
                 train_on=None, thres=None, random_few_shot=None, graph_constraints=None,
                 num_obj_dim=None, pretrain_arch=None, mask_before_frcnn=None,
                 use_pos_enc=None, pre_last_layer=None, mask_type=None,
                 test_on_masked=None, projection_head=None, cosine_loss=None, **kwargs):
        ...
        self.k_shot = k_shot
        self.lb_imgs = lb_imgs
        self.train_on = train_on
        self.thres = thres
        self.random_few_shot = random_few_shot
        self.graph_constraints = graph_constraints
        self.num_obj_dim = num_obj_dim
        self.mask_ratio = mask_ratio
        self.pretrained_model = pretrained_model
        self.pretrain_arch = pretrain_arch
        self.mask_before_frcnn = mask_before_frcnn
        self.pretrain_task = pretrain_task
        self.use_pos_enc = use_pos_enc
        self.pre_last_layer = pre_last_layer
        self.mask_type = mask_type
        self.test_on_masked = test_on_masked
        self.projection_head = projection_head
        self.cosine_loss = cosine_loss
        self.normal_training = normal
        ...
        if finetune:
            self.net_name += '_finetuned'
        else:
            if self.normal_training:
                if not self.lb_imgs:
                    self.net_name += '_finetuned'
                else:
                    self.net_name += f'_{self.lb_imgs}_lb_imgs_finetuned'
            else:
                self.net_name += '_pretrained'
        ...
        self._set_logger()
        if self.use_pos_enc:
            assert self.use_pos_enc == 'learnable' or self.use_pos_enc == 'fixed', 'Argument "use_pos_enc" ' \
            'can be only "fixed" (sin-cos) or learnable (embbeding)'
        if self.mask_before_frcnn:
            self.logger.debug("Masking before Faster-RCNN..")
        if self.projection_head:
            self.logger.debug("Using projection head..")

        assert self.mask_type == 'fixed' or self.mask_type == 'random', "Select type of masking: 'fixed' or 'random'"
